  ...
  return (
    ...
    >
      <div className="relative h-[320px] sm:h-[360px] lg:h-[480px]">
        <img
          src={foto}
          alt={`${nombre}, especialista en ${frase.toLowerCase()}`}
          width="600"
          height="960"
        ...
        <div className="absolute bottom-8 left-8 right-8 rounded-[28px] border border-[#2A2A2A] bg-[#0D0D0D]/70 px-6 py-5 backdrop-blur">
          <h5 className="text-2xl text-[#FAF8F3]">{nombre}</h5>
          <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-[#FAF8F3]/75">{frase}</p>
        </div>
      </div>
    </motion.article>
  );
